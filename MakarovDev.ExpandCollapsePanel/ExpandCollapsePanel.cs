...
namespace MakarovDev.ExpandCollapsePanel
{
    ...
    public partial class ExpandCollapsePanel : Panel
    {
        ...
        ///     <remarks>
        ///         used for handling panel's Anchor property sets to Bottom when panel collapsed
        ...
        private void animationTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (_animationOpacity + 5 < byte.MaxValue) _animationOpacity += 5;
            ...
            // are we done?
            if (_internalPanelState == InternalPanelState.Normal) animationTimer.Enabled = false;
        ...
        private void SetControlsOpacity(int opacity)
        {
            foreach (Control c in Controls)
                if (c.Visible)
                {
                    try
                    {
                        if (c.BackColor != Color.Transparent) c.BackColor = Color.FromArgb(opacity, c.BackColor);
